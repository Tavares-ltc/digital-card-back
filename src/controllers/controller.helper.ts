...
function badRequestResponse(res: Response, text: string | string[] = STATUS_TEXT.BAD_REQUEST) {
...
function okResponse(res: Response, text: any) {
...
function conflictResponse(res: Response, text = "Conflict") {
    return res.status(STATUS_CODE.CONFLICT).send(text);
}

function createdResponse(res: Response, text: any = STATUS_TEXT.CREATED) {
